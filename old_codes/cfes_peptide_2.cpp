...
//Files.................................................
	...
	std::string coord_file_address = getFileAddress(directory, std::string("cfes.xyz"));
...
system("cat hist.dat");
		...
		std::ifstream coord_file(coord_file_address.c_str());
		
		if(i==0){
		hist_file.ignore(std::numeric_limits<std::streamsize>::max(), '\n');//ignore "#"
		hist_file.ignore(std::numeric_limits<std::streamsize>::max(), '\n');//ignore "#"
		}
		coord_file.ignore(std::numeric_limits<std::streamsize>::max(), '\n');//ignore total atoms
		coord_file.ignore(std::numeric_limits<std::streamsize>::max(), '\n');//ignore comment line
...
std::cout << " E " << E << std::endl;
		...
		for(int j = 0; j < TOTAL_ATOMS; j++){
			coord_file >> atom_id >> x[j] >> y[j] >> z[j];
			if(i > 0){
		...
		hist_file.close();
		coord_file.close();
		std::ofstream new_hist_file;
		new_hist_file.open(hist_file_address);
		new_hist_file.close();
		std::ofstream new_coord_file;
		new_coord_file.open(coord_file_address);
